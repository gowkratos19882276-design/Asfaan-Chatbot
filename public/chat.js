(() => {
  ...
  // Keep a simple transcript log for report downloads
  const Transcript = [];
  ...
  function addMessage(text, role = 'bot', buttons = []) {
    ...
    // Save to transcript with timestamp
    try {
      Transcript.push({
  ...
  function buildReportMarkdown() {
    const started = Transcript[0]?.ts || new Date().toISOString();
    const header = `# Chat Transcript\n\n- Started: ${started}\n- Exported: ${new Date().toISOString()}\n- Messages: ${Transcript.length}\n\n---\n`;
    const body = Transcript.map((m, idx) => {
      const t = new Date(m.ts).toLocaleString();
      const who = m.role === 'user' ? 'You' : 'Bot';
      const clean = String(m.text || '').replace(/\r?\n/g, '\n> ');
      return `\n**${idx + 1}. ${who} — ${t}**\n\n> ${clean}\n`;
    }).join('\n');
    return header + body + '\n\n---\nGenerated by Medical Chatbot';
  }
  ...
  function downloadReport() {
    try {
      ...
      a.href = url;
      a.download = `chat-report-${stamp}.md`;
      ...
      addMessage('Failed to generate report. Please try again.', 'bot');
    }
  }
